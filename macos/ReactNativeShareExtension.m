...
- (void)viewDidLoad {
    ...
    NSView *rootView = [self shareView];
...
RCT_EXPORT_METHOD(openURL:(NSString *)url) {
    ...
    NSURL *urlToOpen = [NSURL URLWithString:[url stringByAddingPercentEncodingWithAllowedCharacters:characterset]];
    
    if (@available(macOS 10.15, *)) {
        NSWorkspaceOpenConfiguration *configuration = [NSWorkspaceOpenConfiguration new];
        configuration.arguments = @[];
        [workspace openURL:urlToOpen configuration:configuration completionHandler: nil];
    } else {
